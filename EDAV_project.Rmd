...
####1. feature-wise approach: Parallel coordinate plot by boroughs in New York

In this section, we want to concentrate on the relationships between different numeric variables in listing.csv. We examed that there are several outliers for the numeric variables. In order to check the patterns, we omitted the outliers of price, security deposit, cleaning fee, and accommodates. 
...
```{r, fig.height=18}
ggplot(data = cleaned_nyc_data[,c("longitude","latitude","price","price_label")],
       aes(x=longitude,y=latitude,color = log(price)))+
   ...
   geom_map(data=nyc_districts_map, 
    ...
    ggthemes::theme_map()+
    coord_map() +
    facet_wrap(vars(price_label),ncol = 1, nrow = 3);
```
...
There's even some "mega-host" in NYC,
```{r}
cleaned_nyc_data$host_id = as.factor(cleaned_nyc_data$host_id)
...
2) We can do further analysis on cases with extreme price. 
